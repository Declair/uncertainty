...
def inner_level_loop(cog_pi, inh_p, input_X):
    output_m = sm.run_simu_model(cog_pi, inh_p[0], input_X)
    shape_va = inh_p.shape
    M_v = shape_va[0]
    for i in range(M_v):
        if i == 0:
            continue
        inh_pi = inh_p[i]  # 每一组固有不确定参数
        output_ma = sm.run_simu_model(cog_pi, inh_pi, input_X)  # output_m为仿真模型在固定认知和固有不确定参数下的输出矩阵1*p
...
def outer_level_loop(cog_p, inh_p, output, input_X):  # Es_p为认知不确定参数矩阵N*nr  N为组数，nr为每组的认知不确定性参数个数   Er_p为固有不确定性参数矩阵M*mr M为固有不确定性参数组数，mr为每组固有不确定性参数个数
    ...
    for i in range(N_v):  # 每一组认知不确定参数
        a_mat = inner_level_loop(cog_p[i], inh_p, input_X)
        # print('获得的仿真输出:')
        # print(a_mat)
        # print('认知不确定参数:')
        # print(cog_p[i])
